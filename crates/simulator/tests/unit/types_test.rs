...
fn test_simulation_result_builder() {
    ...
    assert!(revert_result.error_reason.as_ref().unwrap().contains("Insufficient funds"));
    
    // Test failed result with out of gas
    let oog_result = SimulationResultBuilder::failed()
        .with_out_of_gas()
        .build();
        
    assert!(!oog_result.success);
    assert!(oog_result.error_reason.as_ref().unwrap().contains("out of gas"));
    ...
    assert!(nonce_result.error_reason.as_ref().unwrap().contains("Invalid nonce"));
}
...
fn test_large_state_diff() {
    ...
    for i in 0..100 {
        let addr = Address::random();
        for j in 0..10 {
            builder = builder.with_state_change(
                addr,
                U256::from(j),
                U256::from(i * 1000 + j)
            );
...
fn test_error_serialization() {
    ...
    let errors = vec![
        SimulationError::Revert { reason: "test".to_string() },
        SimulationError::OutOfGas,
        SimulationError::InvalidNonce { tx_index: 1, expected: 2, actual: 3 },
        SimulationError::Timeout,
    ];
