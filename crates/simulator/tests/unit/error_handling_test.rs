...
async fn test_simulation_error_types() {
    ...
    let error_scenarios = vec![
        (
            SimulationError::Revert { reason: "Insufficient funds".to_string() },
            "Insufficient funds",
        ),
        (
        ...
        (
            SimulationError::InvalidNonce { tx_index: 0, expected: 5, actual: 3 },
            "Invalid nonce",
        ),
        (
        ...
        (
            SimulationError::StateAccessError { message: "RPC timeout".to_string() },
            "State access error",
        ),
        (
        ...
        (
            SimulationError::Unknown { message: "Unexpected error".to_string() },
            "Unexpected error",
        ),
    ];
    ...
    for (error, expected_msg) in error_scenarios {
        ...
        assert!(published[0].error_reason.as_ref().unwrap().contains(expected_msg));
    }
}
...
async fn test_engine_failure_recovery() {
    ...
    let request1 = SimulationRequestBuilder::new()
        .with_bundle(TestBundleBuilder::new().with_simple_transaction(&[1]).build())
        .build();
    simulator1.simulate(&request1).await.unwrap();
    
    // Test simulation 2: Success
    let engine2 = engine.clone().with_result(SimulationResultBuilder::successful().build());
    let simulator2 = MockBundleSimulator::new(engine2, publisher.clone());
    let request2 = SimulationRequestBuilder::new()
        .with_bundle(TestBundleBuilder::new().with_simple_transaction(&[2]).build())
        .build();
    simulator2.simulate(&request2).await.unwrap();
    
    // Test simulation 3: Revert error
    let engine3 = engine.clone().fail_next_with(SimulationError::Revert { reason: "Test revert".to_string() });
    let simulator3 = MockBundleSimulator::new(engine3, publisher.clone());
    let request3 = SimulationRequestBuilder::new()
        .with_bundle(TestBundleBuilder::new().with_simple_transaction(&[3]).build())
        .build();
    ...
    assert!(!published[0].success);
    assert!(published[0].error_reason.as_ref().unwrap().contains("Bundle ran out of gas"));
    ...
    assert!(published[2].error_reason.as_ref().unwrap().contains("Bundle reverted"));
}
...
fn test_error_display_formatting() {
    ...
    let errors = vec![
        (
            SimulationError::Revert { reason: "ERC20: transfer amount exceeds balance".to_string() },
            "Bundle reverted: ERC20: transfer amount exceeds balance",
        ),
        (
            SimulationError::InvalidNonce { tx_index: 0, expected: 10, actual: 5 },
...
async fn test_timeout_simulation() {
    ...
    let engine = MockSimulationEngine::new()
        .fail_next_with(SimulationError::Timeout);
    let publisher = MockSimulationPublisher::new();
    let simulator = MockBundleSimulator::new(engine, publisher.clone());
    ...
    assert_eq!(published.len(), 1);
    assert!(!published[0].success);
    assert_eq!(published[0].error_reason, Some("Simulation timed out".to_string()));
}
