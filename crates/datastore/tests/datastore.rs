...
fn create_test_state_diff() -> StateDiff {
    ...
    let account1: Address = "0x742d35cc6635c0532925a3b8d40b33dd33ad7309"
        .parse()
        .unwrap();
    let account2: Address = "0x24ae36512421f1d9f6e074f00ff5b8393f5dd925"
        .parse()
        .unwrap();

    // Create storage mappings for account1
    let mut account1_storage = HashMap::new();
    account1_storage.insert(StorageKey::ZERO, StorageValue::from(U256::from(1)));
...
async fn insert_and_get_simulation() -> eyre::Result<()> {
    ...
    // Retrieve simulation
    let retrieved_simulation = harness
        .data_store
    ...
    for (account, expected_storage) in &state_diff {
        ...
        for (slot, expected_value) in expected_storage {
            let actual_value = actual_storage.get(slot).expect("Storage slot should exist");
...
async fn simulation_with_empty_state_diff() -> eyre::Result<()> {
    ...
    assert!(
        simulation.state_diff.is_empty(),
        "State diff should be empty"
    );
...
async fn multiple_simulations_latest_selection() -> eyre::Result<()> {
    ...
    let results = harness
        .data_store
        .select_bundles_with_latest_simulation(BundleFilter::new())
        .await
        .map_err(|e| eyre::eyre!(e))?;
    ...
    assert_eq!(
        latest_sim.block_number, expected_latest_block,
        "Should return simulation with highest block number"
    );
    assert_eq!(
        latest_sim.bundle_id, bundle_id,
        "Should reference correct bundle"
    );
    ...
    assert_eq!(
        latest_sim.execution_time_us, expected_execution_time,
        "Execution time should match latest simulation"
    );
    assert_eq!(
        latest_sim.gas_used, expected_gas_used,
        "Gas used should match latest simulation"
    );

    // Verify the latest simulation has the expected state diff (should be non-empty since i=4 is even)
    assert!(
        !latest_sim.state_diff.is_empty(),
        "Latest simulation should have non-empty state diff"
    );

    // Verify that we can still retrieve all individual simulations
    for (sim_id, block_num, exec_time, gas) in &simulation_ids {
        let individual_sim = harness
            .data_store
...
async fn select_bundles_with_latest_simulation() -> eyre::Result<()> {
    ...
    assert_eq!(
        results.len(),
        2,
        "Should return 2 bundles that have simulations"
    );
...
async fn select_bundles_with_latest_simulation_filtered() -> eyre::Result<()> {
    ...
    let block_filter = BundleFilter::new().valid_for_block(200);
    let filtered_results = harness
        .data_store
        .select_bundles_with_latest_simulation(block_filter)
        .await
        .map_err(|e| eyre::eyre!(e))?;
    ...
    let timestamp_results = harness
        .data_store
        .select_bundles_with_latest_simulation(timestamp_filter)
        .await
        .map_err(|e| eyre::eyre!(e))?;
